...
func Init() {
	...
	fmt.Println("- PostgreSQL (Disabled)")
	fmt.Println("- SQLite (Disabled)")
	fmt.Println("- SQL Server (Disabled)")
	fmt.Println("- MongoDB (Disabled)")
	fmt.Println("\nDatabase API endpoints available at /api/database/*")
	fmt.Println("Note: Only MySQL is configured. Other databases will show as 'error' status.")
	r.Run(":" + port)
}
